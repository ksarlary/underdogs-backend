...
class SyncCurrentUserHandler implements SyncCurrentUser {
    ...
    private User createUserFromJwt(Jwt jwt) {
        ...
        if (username == null || username.isBlank()) {
            throw new BusinessException("MISSING_USERNAME", "Username is missing from the identity provider");
        }

        if (firstName == null || firstName.isBlank()) {
            throw new BusinessException("MISSING_FIRST_NAME", "First name is missing from the identity provider");
        }

        if (lastName == null || lastName.isBlank()) {
            throw new BusinessException("MISSING_LAST_NAME", "Last name is missing from the identity provider");
        }
        ...
        final User user = User.createFromIdentityProvider(
