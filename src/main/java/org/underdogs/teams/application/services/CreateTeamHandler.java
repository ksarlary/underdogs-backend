...
class CreateTeamHandler implements CreateTeam {
    ...
    public TeamId handle(CreateTeamRequest request) {
        teamRepository.findByName(request.name())
                .ifPresent(team -> {
                    throw new BusinessException("TEAM_NAME_ALREADY_EXISTS", "A team with this name already exists");
                });

        teamRepository.findByTag(request.tag())
                .ifPresent(team -> {
                    throw new BusinessException("TEAM_TAG_ALREADY_EXISTS", "A team with this tag already exists");
                });
