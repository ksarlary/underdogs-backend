...
public class TestWorker {
    ...
    public void receiveBetResolutionRequest(ResolveEventBetsRequest request) {
        ...
        List<ResolveEventBetsResponse.BetResult> results = request.payload().bets().stream()
                .map(bet -> {
                    if (bet.predictedTeamId().equals(request.payload().winningTeamId())) {
                        return new ResolveEventBetsResponse.BetResult(
                                ...
                                bet.userId(),
                                "WON",
                                (int) (bet.amountWagered() * bet.odds())
                        );
                    } else {
                        return new ResolveEventBetsResponse.BetResult(
                                bet.betId(),
        ...
        ResolveEventBetsResponse response = new ResolveEventBetsResponse(
